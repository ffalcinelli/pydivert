...
@pytest.mark.skipif(
    not socket.gethostname().startswith("linux") and not socket.gethostname().startswith("ubuntu"),
    reason="Batching test optimized for Linux VM",
)
def test_batch_receive_ebpf():
    ...
    try:
        with EBPFDivert("false") as w:
            ...
            w._impl._queue.extend([p1, p2])  # type: ignore
