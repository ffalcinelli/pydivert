...
from pydivert.util import fromhex
...
async def test_base_final_strike():
    ...
    w.recv_batch(count=1)
    await w.recv_batch_async(count=1)
    w.stats()
    w.close()
    async with FinalConcrete():
        pass
    with FinalConcrete():
        pass
    w4 = FinalConcrete()
...
def test_jit_master_strike():
    p = Packet(b"E" * 20)
    for op in ["+", "-", "*", "/", "%", "<<", ">>", "|", "&", "^"]:
        compile_filter(f"1 {op} 1")(p)
    for op in ["==", "!=", "<", "<=", ">", ">=", "is", "is not", "in", "not in"]:
        compile_filter(f"1 {op} 2" if "in" not in op else f"1 {op} [1,2]")(p)
    compile_filter("True and not False")(p)
    compile_filter("1 if True else 0")(p)
    compile_filter("len(packet.raw)")(p)
    compile_filter("packet.raw[0]")(p)
    compile_filter("[1, 2]")
    compile_filter("(1, 2)")
    compile_filter("{1, 2}")
...
def test_packet_master_strike():
    ...
    raw_ext += (
        b"\x2c\x00\x00\x00\x00\x00\x00\x00"
        + b"\x33\x00\x00\x00\x00\x00\x00\x00"
        + b"\x3c\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
        + b"\x06\x00\x00\x00\x00\x00\x00\x00"
        + b"\x00" * 20
    )
    p_ext = Packet(raw_ext)
    assert p_ext.protocol[0] == 6
