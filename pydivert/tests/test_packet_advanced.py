...
def test_packet_repr_edge_cases():
    ...
    # UDP
    p_udp = pydivert.Packet(b"\x45\x00\x00\x1c\x00\x00\x40\x00\x40\x11\x00\x00\x7f\x00\x00\x01\x7f\x00\x00\x01" + b"\x00\x35\x00\x35\x00\x08\x00\x00")
    assert "UDP" in repr(p_udp)
    
    # ICMP
    p_icmp = pydivert.Packet(b"\x45\x00\x00\x1c\x00\x00\x40\x00\x40\x01\x00\x00\x7f\x00\x00\x01\x7f\x00\x00\x01" + b"\x08\x00\xf7\xff\x00\x00\x00\x00")
