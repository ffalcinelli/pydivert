...
def test_get_last_error_leak_mock():
    ...
    with patch("pydivert.windivert_dll.GetLastError", return_value=error_code):
        ...
        try:
            ...
            pytest.fail(
                f"Bug still present: PyDivert.open() raised [Error {winerror}] "
...
def test_get_last_error_leak_real():
    ...
    try:
        ...
        if winerror == error_code:
            pytest.fail(f"Bug still present: PyDivert.open() raised [Error {winerror}] with real SetLastError(1234)")
        else:
            raise
    finally:
        if w.is_open:
    ...
    try:
        ...
        with patch("pydivert.windivert_dll.GetLastError", return_value=error_code):
            try:
                ...
                winerror = getattr(e, "winerror", None)
                pytest.fail(
                    f"Bug still present: PyDivert.get_param() raised [Error {winerror}] even though call succeeded"
                )
    finally:
        w.close()
