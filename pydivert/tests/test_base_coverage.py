...
def test_base_iterator():
    with pydivert.Divert("false") as w:
        w._impl._recv_impl = MagicMock(side_effect=[pydivert.Packet(b"raw"), EOFError()])
        packets = []
        for p in w:
            packets.append(p)
...
async def test_base_async_iterator():
    async with pydivert.Divert("false") as w:
        ...
        w._impl._recv_async_impl = mock_recv
        packets = []
        async for p in w:
            packets.append(p)
        assert len(packets) == 1
...
def test_base_recv_batch_jit():
    with pydivert.Divert("false") as w:
        ...
        w._impl._recv_batch_impl = MagicMock(return_value=[p1, p2])
...
async def test_base_recv_batch_async_jit():
    async with pydivert.Divert("false") as w:
        ...
        w._impl._recv_batch_async_impl = mock_recv_batch
