...
wd_dir = os.path.join(os.path.dirname(__file__), os.pardir, "../lib")


class WinDivertDLLLoadTestCase(unittest.TestCase):

    def setUp(self):
        os.chdir(wd_dir)
        self.dll_path = os.path.join(wd_dir, "WinDivert.dll")
...
class WinDivertTestCase(unittest.TestCase):
    ...
    def setUp(self):
        os.chdir(wd_dir)
        self.wd = WinDivert(os.path.join(wd_dir, "WinDivert.dll"))

    def test_open_handle(self):
        """
        Test the open_handle method.
        """
        handle = self.wd.open_handle(filter="tcp.DstPort == 23", priority=1000)
    ...
    def test_construct_handle(self):
        ...
        """
        handle = Handle(self.wd, filter="tcp.DstPort == 23", priority=1000)
        self.assertIsInstance(handle, Handle)
        self.assertFalse(handle.is_opened)
    ...
    def test_context_manager(self):
        ...
        """
        with Handle(self.wd, filter="tcp.DstPort == 23", priority=1000) as filter0:
    ...
    def test_parse_ipv4_address(self):
        ...
        """
        address = "192.168.1.1"
        result = self.wd.parse_ipv4_address(address)
    ...
    def test_parse_ipv6_address(self):
        ...
        result = self.wd.parse_ipv6_address(address)
...
class WinDivertTCPCaptureTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.driver = WinDivert(os.path.join(wd_dir, "WinDivert.dll"))
    ...
    def test_pass_through(self):
        ...
        self.client_thread.join(timeout=30)
    ...
    def test_packet_checksum(self):
        ...
        raw_packet2 = self.driver.calc_checksums(raw_packet1)
...
class WinDivertTCPInjectTestCase(unittest.TestCase):
    ...
    def test_packet_checksums(self):
        ...
        if metadata.direction == enum.DIVERT_DIRECTION_OUTBOUND:
            ...
            raw_packet = self.driver.calc_checksums(packet.to_raw_packet())
